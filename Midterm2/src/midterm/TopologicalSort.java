...
//      if (!visited[at])
//        i = dfs(numNodes-1, at, visited, ordering, graph);
//
//    return ordering;
//
//  }

    public static int dfs(int pos, int at, Map<Integer, List<Edge>> graph, int[] ordering, boolean[] visited){
        visited[at] = true;
        List<Edge> edges = graph.get(at);
        if(edges != null){
            for(Edge edge : edges){
                if(!visited[edge.to]){
                   pos = dfs(pos, edge.to, graph, ordering, visited);
                }
            }
        }
  ...
  public static int[] topologicalSort(Map<Integer, List<Edge>> graph, int numNodes){
      ...
      for(int at = 0; at < numNodes; at++){
          if(!visited[at]){
              position = dfs(position, at, graph, ordering, visited);
...
//    graph.get(5).add(new Edge(5,4,7));
      ...
      graph.get(0).add(new Edge(3,1,2));
      graph.get(0).add(new Edge(3,0,2));
      graph.get(1).add(new Edge(0,2,2));
      graph.get(2).add(new Edge(1,2,2));
